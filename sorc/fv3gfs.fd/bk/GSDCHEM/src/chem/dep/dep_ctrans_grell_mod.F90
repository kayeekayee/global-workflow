...
!  jpr=60
   ...
   jtf=jte !lzhang
!                                                                      
       write(0,*),'zlzlstart',maxval(chem(:,:,:,:)),minval(chem(:,:,:,:))
!
!    write(6,*)'in ctrans'
     trfall(:,:,:)=0.
123  continue
...
!---- CALL NON_RESOLVED CONVECTIVE TRANSPORT
       write(0,*),'zlzl0',j,maxval(tracer(:,:,:)),minval(tracer(:,:,:))
       write(0,*),'zlzl00',maxval(tracert(:,:,:)),minval(tracert(:,:,:))
!
      CALL CUP_gf(kpbli,ktop,tracer,trdep,aaeq,t,q,ter11,zz,pret,p,tracert,    &
           hstary,dt,psur,us,vs,tcrit,num_chem,chem_opt,numgas,      &
           rho,xlv,r_v,cp,g,ipr,its,ite, kts,kte,csum )
       write(0,*),'zlzl1',maxval(tracer(:,:,:)),minval(tracer(:,:,:))
       write(0,*),'zlzl11',maxval(tracert(:,:,:)),minval(tracert(:,:,:))
            do nv=1,num_chem
            DO I=its,itf
              if(pret(i).le.0.)then
                 DO K=kts,ktf
...
!lzhang diag trdep:
       ...
       write(0,*),'zlzl2',maxval(tracer(:,:,:)),minval(tracer(:,:,:))
       write(0,*),'zlzl22',maxval(tracert(:,:,:)),minval(tracert(:,:,:))
       ...
       do nv=1,num_chem
            ...
            ENDDO
       ENDDO
       write(0,*),'zlzl33',maxval(tracert(:,:,:))*dt,minval(tracert(:,:,:))*dt


 100    continue
       write(0,*),'zlzle3',maxval(chem(:,:,:,:)),minval(chem(:,:,:,:))
...
!
   ...
   IMPLICIT NONE
     ...
     integer                                                           &
        ,intent (in   )                   ::                           &
        numc,chem_opt,numgas,its,ite,  kts,kte,ipr
  !
  ! 
  !
  ...
  !
     ...
     integer,    dimension (its:ite), intent(in) :: kpbli,csum
...
!
      ...
      do i=its,itf
        ...
        dd_massdetr(i,:)=0.
        dd_massentr(i,:)=0.
           call get_zu_zd_pdf_fim(zuh2,"DOWN",ierr(i),kdet(i),jmin(i),zd(i,:),kts,kte,ktf,beta,kpbli(i),csum(i))
...
!                                 zu(i,k  )* qrco(i,k  )  )*g/dp + G_rain
               ...
               dellac(i,k,nv) =-(zu(i,k+1)*(chem_up(i,k+1,nv)-chem_cup(i,k+1,nv) ) - &
                    ...
                    zd(i,k  )*(chem_down(i,k,nv  )-chem_cup(i,k,nv)))*g/dp*edt(i) &
                         - g_rain + E_dn
         if (dellac(i,k,nv).ge.23922.) then
             write(0,*),'zlzlde0',dellac(i,k,nv),i,k,nv
             write(0,*),'zlzlde1',zu(i,k+1),chem_up(i,k+1,nv),chem_cup(i,k+1,nv)
             write(0,*),'zlzlde2',zu(i,k  ),chem_up (i,k,nv ),chem_cup(i,k,nv),dp,zd(i,k+1)
             write(0,*),'zlzlde3',chem_down(i,k+1,nv),chem_cup(i,k+1,nv),zd(i,k),chem_down(i,k,nv)
             write(0,*),'zlzlde4', chem_cup(i,k,nv),edt(i),g_rain,E_dn
             write(0,*),'zlzlde5', up_massdetr(i,k),up_massentr(i,k),  &
                       dd_massdetr(i,k),dd_massentr(i,k)
             write(0,*),'zlzlde6', up_massdetr(i,k+1),up_massentr(i,k+1) , &
                       dd_massdetr(i,k+1),dd_massentr(i,k+1)
             write(0,*),'zlzlde7', chem_c(i,k,nv)
         endif
            enddo
       ...
       write(0,*),'zlzlcu7',maxval(dellac(:,:,:)),minval(dellac(:,:,:))
...
!
       ...
       write(0,*),'zlzlcu8',maxval(outc(:,:,:)),minval(outc(:,:,:))
...
!
            DO k=kbcon(i),ktop(i)
               denom=zu(i,k-1)-.5*up_massdetr(i,k-1)+up_massentr(i,k-1)
               if(denom.lt.1.e-8)then
!          write(14,*)j,k,denom,zu(i,k-1),up_massdetro(i,k-1),up_massentro(i,k-1)
                     ierr(i)=51
                exit
               endif
...
!
       ...
       if( name == 'deep' ) then
          if(kfinalzu.le.kbcon(i)+2)then
           ...
           call get_zu_zd_pdf_fim(zuh2,"UP",ierr(i),k22(i),kfinalzu,zu(i,kts:kte),kts,kte,ktf,0.,kpbli(i),csum(i))
          endif
       elseif ( name == 'mid' ) then
!         kfinalzu=min(kfinalzu,kstabi(i)+1)
!         ktopdby(i)=kfinalzu
!         ktop(i)=kfinalzu+1
          call get_zu_zd_pdf_fim(zuh2,"MID",ierr(i),k22(i),kfinalzu,zu(i,kts:kte),kts,kte,ktf,0.,kpbli(i),csum(i))
           kbegin=0
           ...
           do k=k22(i)+1,kfinalzu+1
             ...
             enddo
           enddo
           if(zu(i,ktop(i)).lt.1.e-4)ktop(i)=ktop(i)-1
!          if(jpr.eq.1)then
!
!!sms$ignore begin
!           do k=kts,kfinalzu
!             write(12,*)k,zu(i,k),zuh2(k)
!           enddo
!           call flush(12)
!!sms$ignore end
!          endif
       elseif ( name == 'shallow' ) then
          if(kfinalzu.le.kbcon(i)+1)then
           ...
           call get_zu_zd_pdf_fim(zuh2,"SH3",ierr(i),k22(i),kfinalzu,zu(i,kts:kte),kts,kte,ktf,0.,kpbli(i),csum(i))
           kbegin=0
...
!-------------------------------------------------------------------------
subroutine get_zu_zd_pdf_fim(zuh2,draft,ierr,kb,kt,zu,kts,kte,ktf,max_mass,kpbli,csum)

implicit none
integer, intent(in) ::kb,kt,kts,kte,ktf,kpbli,csum
real, intent(in) ::max_mass
real, intent(inout) :: zu(kts:kte),zuh2(1:40) !lzhang
real  :: zuh(kts:kte)!,zuh2(1:40)  lzhang

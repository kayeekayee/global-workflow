...
!$OMP             private (j, jb, i, ib, nb, ix)
      do j = 1, nj
        ...
        do i = 1, ni
          ...
          nb = Atm_block%blkno(ib,jb)
          ix = Atm_block%ixp(ib,jb)
          IPD_Data(nb)%coupling%rainc_cpl(ix)  = 0.0_IPD_kind_phys
          if (.not.IPD_Control%cplflx) then
            IPD_Data(nb)%coupling%rain_cpl(ix) = 0.0_IPD_kind_phys
            IPD_Data(nb)%coupling%snow_cpl(ix) = 0.0_IPD_kind_phys
          end if
        enddo
      enddo
...
!
    ...
    real(kind=IPD_kind_phys), parameter                :: icemin=1.0e-6_IPD_kind_phys
!   real(kind=IPD_kind_phys), parameter                :: icemin=0.15_IPD_kind_phys

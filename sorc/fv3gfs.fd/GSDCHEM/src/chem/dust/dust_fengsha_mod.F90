...
contains
  ...
  subroutine gocart_dust_fengsha_driver(dt,              &
    ...
    real(CHEM_KIND_R8), DIMENSION (1,1) :: erodtot
  ...
  subroutine fengsha_utst(styp,uth, ut)
    ...
    real, dimension(fengsha_maxstypes), intent(in)  :: uth
    real(CHEM_KIND_R8),                 intent(out) :: ut
    ut = uth(styp)
!     real (CHEM_KIND_R8) :: uth(13) = &
!          (/ 0.08,   & ! Sand          - 1
!          0.20,    & ! Loamy Sand      - 2
!          0.30,    & ! Sandy Loam      - 3
!          0.30,    & ! Silt Loam       - 4
...
!          9.999 /)   ! Other           - 13
  ...
  subroutine calc_fengsha_styp(clay, sand, silt, type)
    ...
    !---------------------------------------------------------------
    REAL(CHEM_KIND_R8), intent(in) ::  clay, sand, silt
    integer, intent(out) ::  type
    real(CHEM_KIND_R8) :: cly, snd, slt
  ...
  subroutine fengsha_hflux(ust,utst, kvh, salt)
    ...
    !
    !--------------------------------------------------------------------
    real(CHEM_KIND_R8), intent(in) :: ust, & ! friction velocity
                                     utst, & ! threshold friction velocity
                                      kvh    ! vertical to horizontal mass flux ratio

    real(CHEM_KIND_R8), intent(out) :: salt
    real(CHEM_KIND_R8) :: Q

...
taskplan = ['getic', 'init', 'fcst', 'post', 'arch']
...
def get_workflow(dict_configs, cdump='gdas'):
    ...
    if app in ['S2S', 'S2SW']:
        ...
        if hpssarch in ['YES']:
            ...
            data = '&PUBDIR;/@y@j@H00.&CDUMP;.t@Hz.sfcanl.nc'
            dep_dict = {'type':'data', 'data':data}
            deps3.append(rocoto.add_dependency(dep_dict))
            dependencies3 = rocoto.create_dependency(dep_condition='and', dep=deps3)

            deps4 = []
            data = '&RETRODIR;/@y@j@H00.&CDUMP;.t@Hz.atmanl.nc'
            dep_dict = {'type':'data', 'data':data}
            deps4.append(rocoto.add_dependency(dep_dict))
            data = '&RETRODIR;/@y@j@H00.&CDUMP;.t@Hz.sfcanl.nc'
            dep_dict = {'type':'data', 'data':data}
            deps4.append(rocoto.add_dependency(dep_dict))

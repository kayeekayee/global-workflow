...
#JKHtaskplan = ['getic', 'init', 'coupled_ic', 'waveinit', 'waveprep', 'fcst', 'post', 'wavepostsbs', 'wavepostbndpnt', 'wavepostbndpntbll', 'wavepostpnt', 'wavegempak', 'waveawipsbulls', 'waveawipsgridded', 'wafs', 'wafsgrib2', 'wafsblending', 'wafsgcip', 'wafsgrib20p25', 'wafsblending0p25', 'postsnd', 'gempak', 'awips', 'vrfy', 'metp', 'arch', 'ocnpost']
taskplan = ['init', 'fcst', 'post', 'arch']
...
def get_definitions(base):
    ...
    strings.append(f'''\t<!ENTITY COMPONENT  "atmos">\n''')
    strings.append(f'''\t<!ENTITY ATCFNAME   "">\n''')
    strings.append('\n')
    ...
    strings.append(f'''\t<!ENTITY PUBDIR "/scratch2/BMC/public/data/grids/gfs/anl/netcdf">\n''')
    strings.append(f'''\t<!ENTITY EMCDIR "/scratch1/NCEPDEV/rstprod/com/gfs/prod">\n''')
    strings.append(f'''\t<!ENTITY RETRODIR "/scratch1/BMC/gsd-fv3-dev/GFS_RETRO_NETCDF">\n''')
    strings.append('\n')
...
def get_workflow(dict_configs, cdump='gdas'):
    ...
    do_vrfy = base.get('DO_VRFY', 'NO').upper()          ## JKH
    do_metp = base.get('DO_METP', 'NO').upper()
...
##JKH        if hpssarch in ['YES']:
##JKH            deps1 = []
##JKH            data = '&ROTDIR;/&CDUMP;.@Y@m@d/@H/atmos/INPUT/sfc_data.tile6.nc'
##JKH            dep_dict = {'type': 'data', 'data': data}
##JKH            deps1.append(rocoto.add_dependency(dep_dict))
##JKH            data = '&ROTDIR;/&CDUMP;.@Y@m@d/@H/atmos/RESTART/@Y@m@d.@H0000.sfcanl_data.tile6.nc'
##JKH            dep_dict = {'type': 'data', 'data': data}
##JKH            deps1.append(rocoto.add_dependency(dep_dict))
##JKH            dependencies1 = rocoto.create_dependency(dep_condition='nor', dep=deps1)
##JKH
##JKH            deps2 = []
##JKH            data = '&EMCDIR;/&CDUMP;.@Y@m@d/@H/&COMPONENT;/&CDUMP;.t@Hz.atmanl.nc'
##JKH            dep_dict = {'type':'data', 'data':data}
##JKH            deps2.append(rocoto.add_dependency(dep_dict))
##JKH            data = '&EMCDIR;/&CDUMP;.@Y@m@d/@H/&COMPONENT;/&CDUMP;.t@Hz.sfcanl.nc'
##JKH            dep_dict = {'type':'data', 'data':data}
##JKH            deps2.append(rocoto.add_dependency(dep_dict))
##JKH            dependencies2 = rocoto.create_dependency(dep_condition='and', dep=deps2)
##JKH
##JKH            deps3 = []
##JKH            data = '&PUBDIR;/@y@j@H00.&CDUMP;.t@Hz.atmanl.nc'
##JKH            dep_dict = {'type':'data', 'data':data}
##JKH            deps3.append(rocoto.add_dependency(dep_dict))
##JKH            data = '&PUBDIR;/@y@j@H00.&CDUMP;.t@Hz.sfcanl.nc'
##JKH            dep_dict = {'type':'data', 'data':data}
##JKH            deps3.append(rocoto.add_dependency(dep_dict))
##JKH            dependencies3 = rocoto.create_dependency(dep_condition='and', dep=deps3)
##JKH
##JKH            deps4 = []
##JKH            data = '&RETRODIR;/@y@j@H00.&CDUMP;.t@Hz.atmanl.nc'
##JKH            dep_dict = {'type':'data', 'data':data}
##JKH            deps4.append(rocoto.add_dependency(dep_dict))
##JKH            data = '&RETRODIR;/@y@j@H00.&CDUMP;.t@Hz.sfcanl.nc'
##JKH            dep_dict = {'type':'data', 'data':data}
##JKH            deps4.append(rocoto.add_dependency(dep_dict))
##JKH            dependencies4 = rocoto.create_dependency(dep_condition='and', dep=deps4)
##JKH
##JKH            deps = []
##JKH            deps = dependencies2
##JKH            deps.append(dependencies3)
##JKH            deps.append(dependencies4)
##JKH            dependencies = rocoto.create_dependency(dep_condition='or', dep=deps)
##JKH
##JKH            deps = []
##JKH            deps.append(dependencies1)
##JKH            deps.append(dependencies)
##JKH            dependencies = rocoto.create_dependency(dep_condition='and', dep=deps)
##JKH
##JKH            PUBDIR = rocoto.create_envar(name='PUBDIR', value='&PUBDIR;')
##JKH            EMCDIR = rocoto.create_envar(name='EMCDIR', value='&EMCDIR;')
##JKH            RETRODIR = rocoto.create_envar(name='RETRODIR', value='&RETRODIR;')
##JKH            ROTDIR = rocoto.create_envar(name='ROTDIR', value='&ROTDIR;')
##JKH            PSLOT = rocoto.create_envar(name='PSLOT', value='&PSLOT;')
##JKH            COMPONENT = rocoto.create_envar(name='COMPONENT', value='&COMPONENT;')
##JKH            geticenvars = envars + [PUBDIR] + [EMCDIR] + [RETRODIR] + [ROTDIR] + [PSLOT] + [COMPONENT]
##JKH            task = wfu.create_wf_task('getic', cdump=cdump, envar=geticenvars, dependency=dependencies)
...
##JKH            tasks.append('\n')
        ...
        ICSDIR = rocoto.create_envar(name='ICSDIR', value='&ICSDIR;')
        ROTDIR = rocoto.create_envar(name='ROTDIR', value='&ROTDIR;')
        COMPONENT = rocoto.create_envar(name='COMPONENT', value='&COMPONENT;')
        CASE = rocoto.create_envar(name='CASE', value='&CASE;')
        initenvars = envars + [ICSDIR] + [ROTDIR] + [COMPONENT] + [CASE]
        task = wfu.create_wf_task('init', cdump=cdump, envar=initenvars, dependency=dependencies)
        tasks.append(task)
        tasks.append('\n')
    ...
    dep_dict = {'type':'data', 'data':data}
    deps.append(rocoto.add_dependency(dep_dict))
    data = '&ROTDIR;/&CDUMP;.@Y@m@d/@H/&COMPONENT;/INPUT/aero_done'
    dep_dict = {'type':'data', 'data':data}
    deps.append(rocoto.add_dependency(dep_dict))
    data = '&ROTDIR;/&CDUMP;.@Y@m@d/@H/&COMPONENT;/INPUT/chgres_done'
    dep_dict = {'type':'data', 'data':data}
    deps.append(rocoto.add_dependency(dep_dict))
    dependencies2 = rocoto.create_dependency(dep_condition='or', dep=deps)

    deps = []
    data = '&ROTDIR;/&CDUMP;.@Y@m@d/@H/&COMPONENT;/INPUT/sfc_data.tile6.nc'
    dep_dict = {'type':'data', 'data':data}
    deps.append(rocoto.add_dependency(dep_dict))
    data = '&ROTDIR;/&CDUMP;.@Y@m@d/@H/&COMPONENT;/INPUT/gfs_data.tile6.nc'
    dep_dict = {'type':'data', 'data':data}
    deps.append(rocoto.add_dependency(dep_dict))
    ...
    dependencies = rocoto.create_dependency(dep_condition='and', dep=deps)
    COMPONENT = rocoto.create_envar(name='COMPONENT', value='&COMPONENT;')
    archenvars = envars + [COMPONENT]
    task = wfu.create_wf_task('arch', cdump=cdump, envar=archenvars, dependency=dependencies, final=True)
    tasks.append(task)
    tasks.append('\n')

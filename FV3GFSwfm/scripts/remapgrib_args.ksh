...
if [ $# -lt 4 ]; then
  echo "Usage:  $0 yyjjjhhmm yyyymmdd  hh fcst"
  exit 1
fi
...
FV3GFS_RUN=/scratch4/BMC/rtfim/rtruns/FV3GFS/FV3GFSrun
PSLOT=rt_fv3gfs_ff
CDUMP=gfs
#yyjjjhhmm=180600000
#yyyymmdd=20180301
...
src_gribfile=gfs.t00z.pgrb2.0p25.f${fcst}
...
post_dir=${FV3GFS_RUN}/${PSLOT}/${CDUMP}.${yyyymmdd}/${hh}/post/
cd $post_dir
for grid in $grids
do
    tgt_gribfile_dir=${post_dir}/${grid}
    ${MKDIR} -p ${tgt_gribfile_dir}
    tgt_gribfile=${tgt_gribfile_dir}/${yyjjjhhmm}0${fcst}.g2
    ${ECHO} "Processing grids for grid ${grid}"
    eval grid_specs=\${grid_specs_${grid}}
    ${WGRIB2} ../${src_gribfile} -set_grib_type c3 -new_grid_winds grid \
